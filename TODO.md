...
- [X] 修正gpt問完問題會自己安排task的毛病（可能是gpt太笨 先暫時忽略）
- [X] 加上刪除功能
- [X] 加上修改功能
...
- [X] 刪除匡的顯示資訊有問題
- [X] 新增任務要依照現在時間
- [X] 新增任務匡的展開收起有問題（畫面不會跟著動 新增的確認匡有時候收起會讓畫面的泡泡不見 雖然往上滑動又好了）
- [X] gpt打字時畫面不會跟著動
- [X] 執行函式的時候載入動畫頁要持續在跑
- [X] 如果gpt講話到一半使用者突然講話要中斷gpt的對話
- [X] 測試新增刪除修改對類別統計的影響
- [X] 會重複gettask
- [X] 如果有任務確認匡的話畫面還是會結束對話又從上面跳到下面
- [X] 停止按鈕要變大
- [X] 要可以修改標題文字
- [X] 標題要稍微往右移
- [X] 把輸入匡之上的陰影修淡
...
- [x] 有時候切過來會閃白
- [ ] 跨天任務如果有新一個禮拜的話要可以重新排疊層不然會出現第一排為空
- [ ] 週月重複的跨天辦法顯示 只有第一天會顯示而已
- [ ] 下個月跟上個月的任務不一定會正常顯示 點了還會跳到其他頁
- [X] 每週每月每日只會重複在第一個禮拜

## todoview

- [ ] 滑動Todo的編輯要關起鍵盤
- [X] 每週每月每日與跨日任務要新增一個集合去修正打勾一天就會整個重複都打勾

## todoadd todoedit

- [ ] 開始時間輸入完沒有跳到結束時間
- [ ] 每日每週每月沒有限制選擇時間範圍（要顯示可以選多長的時間）
