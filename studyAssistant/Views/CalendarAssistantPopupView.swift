...
struct CalendarAssistantPopupView: View {
    ...
    private func insertDefaultText() {
        ...
        【階段二：識別連續同類任務鏈】
        在「待重排」任務中，識別「連續任務鏈」：
        - 連續任務鏈定義：相同標題或類別、連續多天（間隔≤1天）、時段相同或相近（開始時間差異≤30分鐘）

        【階段三：智能時間分配 - 鏈式遞延】
        若任務屬於「連續任務鏈」：
        1. 找出鏈中第一個「未完成且過期」的任務（待補任務）
        2. 嘗試將「待補任務」移到今天的原時段
        3. 若今天原時段被該鏈的後續任務佔用：
           - 將今天的任務移到明天、明天的移到後天，以此類推
           - 將「待補任務」插入今天的原時段
        4. 鏈的最後一個任務若需順延：
           - 檢查下一天原時段是否被佔用
           - 若被佔用，改用一般邏輯尋找其他空檔
        ...
        【階段四：一般任務安排】
        對於非連續鏈任務：
        1. 遵守 StudySettings 的每日可用時段（預設 08:00-22:00）
        2. 尋找完全適配的空檔（任務時長 ≤ 空檔時長）
        3. 若無法適配且為讀書類，可拆分任務（標題加 [1/2] [2/2]）
        4. 若當天無空檔，順延到下一天
        5. 運動類任務優先安排在早晨（7:00-9:00）或傍晚（18:00-20:00）
